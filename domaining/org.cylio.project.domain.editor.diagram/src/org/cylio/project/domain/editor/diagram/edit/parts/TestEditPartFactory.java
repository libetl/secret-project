    ...
    if (model instanceof View) {
      ...
      switch (TestVisualIDRegistry.getVisualID (view)) {
        ...
        case InformationSystemEditPart.VISUAL_ID :
          return new InformationSystemEditPart (view);

        case SubDomainEditPart.VISUAL_ID :
          return new SubDomainEditPart (view);

        case SubDomainNameEditPart.VISUAL_ID :
          return new SubDomainNameEditPart (view);
        ...
        case SubDomain2EditPart.VISUAL_ID :
          return new SubDomain2EditPart (view);

        case SubDomainName2EditPart.VISUAL_ID :
          return new SubDomainName2EditPart (view);
  ...
  private static class LabelCellEditorLocator implements CellEditorLocator {
    ...
    public void relocate (CellEditor celleditor) {
      ...
      if (!text.getFont ().isDisposed ()) {
        int avr = FigureUtilities.getFontMetrics (text.getFont ())
            .getAverageCharWidth ();
        rect.setSize (new Dimension (text
            .computeSize (SWT.DEFAULT, SWT.DEFAULT)).expand (avr * 2, 0));
      }
      if (!rect.equals (new Rectangle (text.getBounds ()))) {
        text.setBounds (rect.x, rect.y, rect.width, rect.height);
      }
