package org.cylio.project.domain.editor.diagram.parsers;
...
public abstract class AbstractParser implements IParser {
  ...
  public AbstractParser (EAttribute[] features, EAttribute[] editableFeatures) {
    ...
    if (editableFeatures == null
        || Arrays.asList (editableFeatures).contains (null)) { throw new IllegalArgumentException (); }
  ...
  public boolean isAffectingEvent (Object event, int flags) {
    if (event instanceof Notification) { return isAffectingFeature (((Notification) event)
        .getFeature ()); }
    return false;
  }
  ...
  protected ICommand getParseCommand (IAdaptable adapter, Object[] values,
      int flags) {
    if (values == null
        || validateNewValues (values).getCode () != IParserEditStatus.EDITABLE) { return UnexecutableCommand.INSTANCE; }
  ...
  protected IParserEditStatus validateNewValues (Object[] values) {
    ...
    for (int i = 0 ; i < values.length ; i++) {
      ...
      if (value instanceof InvalidValue) { return new ParserEditStatus (
          TestDiagramEditorPlugin.ID, IParserEditStatus.UNEDITABLE,
          value.toString ()); }
    }
